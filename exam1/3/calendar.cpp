...
void Calendar::timeOut() {
    for (int i = 0; i < m_things.count(); i++) {
        if (m_things.at(i).isValidToday(QDate::currentDate()) && m_things.at(i).getTime().hour() == QTime::currentTime().hour() && m_things.at(i).getTime().minute() == QTime::currentTime().minute() && m_things.at(i).getTime().second() == QTime::currentTime().second()) {
            QMessageBox::information(this, "Todo", m_things.at(i).getTodoThing() + " should be done!");
        }
    }
}
...
{
//    emit (backgroundColorChanged(date, Qt::white));
